      ...
      this.redisClient.get(`${this.options.prefix}:${key}`, (err, reply) => {
        if (err) {
          ...
          if (this.options.debug) {
            console.error(`jfetchs-redis/src/index.ts:88${prefix} error`, err)
          }
          return
        }
        if (reply === null) {
        ...
        try {
          ...
          if (this.options.debug) {
            console.error(`jfetchs-redis/src/index.ts:101${prefix} error`, err)
          }
          reject(err)
          return
        }
    ...
    return new Promise((resolve, reject) => {
      this.redisClient.setex(
        ...
        err => {
          if (err) {
            if (this.options.debug) {
              console.error(`jfetchs-redis/src/index.ts:132${prefix} error`, err)
            }
            reject(err)
            return
          }
  ...
  remove(key: string): Promise<boolean> {
    ...
    return new Promise((resolve, reject) => {
      this.redisClient.del(`${this.options.prefix}:${key}`, (err, reply) => {
        if (err) {
          if (this.options.debug) {
            console.error(`jfetchs-redis/src/index.ts:158${prefix} error`, err)
          }
          reject(err)
          return
        }
